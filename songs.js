...
class Songs {
	...
	async removeSongByQueuePosition(props) {
		console.log('removing by queue position');
		props.ignoreMessageParamsForUserString = true;
		const propsForSelect = {
			table: 'songs',
			query: {channel: props.channel}
			...
			for (const songToRemove of songsToRemove) {
				for (let i = 0; i < results.length; i++) {
					if (i == (songToRemove - 1)) {
						...
						deletedResults.push(database.delete(propsForDelete));
						atLeastOneSongRemoved = true;
						break;
					}
				}
			}
			await Promise.all(deletedResults);
	...
	async checkCacheTimeCheck(props) {
		...
		if (results) {
			...
			throw new Error('failed toosoon');
		} else {
			return 'good';
		}
	}
	...
	checkCountryRestrictions(props) {
		if (props.YTData[0].allowedRegions && props.YTData[0].allowedRegions.length != 0) {
			...
			throw new Error('failed countryCheck');
		} else {
			return 'good';
		}
	}
	...
	checkIfSongCanBeEmbeded(props) {
		...
		throw new Error('failed embedCheck');
	}
	...
	async requestCommaListOfSongs(props) {
		...
		for (let x = 0; x < props.songsToAdd.length - 1; x++) {
			try {
				...
				props.songToAdd = youTubeID;
				addSongArray.push(this.addSongWrapper(props));
			} catch (err) {
				if (err == 'failed limit') {
					break;
				}
		...
		await Promise.all(addSongArray);
		socket.io.in(functions.stripHash(props.channel)).emit('songs', ['added']);
		return await this.buildMessageToSendForAddSong(props) + '!';
	}
	...
	async getChannelSongNumberLimit(props) {
		...
		throw new Error('failed getChannelSongNumberLimit');
	}
	...
	async getChannelMaxSongLength(props) {
		...
		throw new Error('failed getChannelMaxSongLength');
	}
	...
	async getYouTubeVideoIDFromChatMessage(songToAdd) {
		...
		if (songToAdd.length == 11 && !songToAdd.includes(' ')) {
			...
			throw new Error('Invalid URL! To request a song, type the following: !sr youtube URL, video ID, or the song name');
			...
			if (numberOfResults > 0) {
				...
				throw new Error('Song not found! To request a song, type the following: !sr youtube URL, video ID, or the song name');
			} else {
				throw new Error('Song not found! To request a song, type the following: !sr youtube URL, video ID, or the song name');
			}
		}
